#!/Users/me/Desktop/coin_py/bin/python3
...
def predict_price(ticker):
    ...
    model = Prophet()
    model.fit(data)
    future = model.make_future_dataframe(periods=4, freq='H') # 4시간마다 리셋되니 4시간 후 까지만 예측하면 됨
    forecast = model.predict(future)
    endhour = int((get_start_time(coin_name) + datetime.timedelta(hours=4)).strftime("%H")) # 종가 시간 추출
...
schedule.every(15).minute.do(lambda: predict_price(coin_name)) # 15분마다 예측 돌리기
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            #로그 출력
            print( date_time + " [ " + coin_name + " ] 목표 : " + str(target_price) + ", 현재 : " + str(current_price) + ", 예측 : " + str(predicted_close_price) + ", 매수평균 : " + str(blended_price))

            if target_price < current_price < predicted_close_price: # 구매조건 ; 현재가격이 예측가격보다 작고, 목표금액보다 클 경우
            #if True: # for debug
                #krw = get_balance("KRW")
            ...
            for b in balances:
                if b['currency'] == coin_name:
                    if b['balance'] is not None:
                        coin2krw_now = decimal.Decimal(b['balance']) * decimal.Decimal(b['avg_buy_price']) # 현재 코인 구매 평균가 x 현재 코인 개수
                        balance = decimal.Decimal(b['balance']) # 코인갯수
                        # print(str( coin2krw_now )) # 현재 코인의 원화 가치

            if coin2krw_now > 5000 and current_price > blended_price > 0: # 가지고 있는 코인이 원화 5천원 이상이면 판매, 매수평균가 보다 현재 가격이 높아야 함
                sell = upbit.sell_market_order(coin_name, balance * decimal.Decimal(0.9995)) # 팔때 총 갯수와 구매 평균가격 확인
                print(sell) # 팔리면 정보 프린트
                blended_price = 0
